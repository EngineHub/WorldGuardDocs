...
.. csv-table::
    ...
    use-max-priority-association,FALSE,"The region membership of non-player associables, such as pistons, is determined based on the set of regions in which they are located, including all the parent regions. This setting controlls wheter or not non-player associables are only members of the regions with the highest priorities in the described set. If set to ``false``, non-player associables are members of all regions in the described set, which means that pistons, for example, can push blocks from inside a region outwards into a surrounding region (unless there is only a protected :doc:`regions/global-region` outside). If set to ``true``, pistons, for example, cannot push blocks from inside a region outwards into a surrounding protected region with a lower priority."
