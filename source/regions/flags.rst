...
.. csv-table::
    ...
    * Why not set ``build`` to ``allow`` (explained later) instead? That would override other regions and let people build!"
    nonplayer-protection-domains,set of strings,"The region membership of non-player associables, such as pistons, is determined based on the set of regions in which they are located, including all the parent regions. Non-player associables are usually members of either all regions in the described set (and their children due to :doc:`priorities`) or only of the regions with the highest priorities (and the children of those regions) in the described set, depending on the ``use-max-priority-association`` :doc:`../config` setting and if it is a :doc:`global-region`. Thus there can be borders between regions, such that pistons cannot move blocks from one region into another region.
